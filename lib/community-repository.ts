import type { CommunityUser, CyclingRoute, RideActivity } from "@/lib/community-types";
...
type ActivityRow = {
  id: string; route_id: string; route_name: string; route_color: string; title: string; details: string;
  starts_at: string; meeting_name: string; meeting_longitude: number; meeting_latitude: number;
  capacity: number; pace: RideActivity["pace"]; status: RideActivity["status"];
  creator_id: string; username: string; display_name: string; avatar_color: string;
  joined_count: number; joined_by_me: number;
};

export async function listRideActivities(db: D1Database, user: CommunityUser | null): Promise<RideActivity[]> {
  const rows = await db.prepare(
    `SELECT a.id, a.route_id, r.name AS route_name, r.color AS route_color, a.title, a.details,
     ...
     FROM activities a JOIN users u ON u.id = a.creator_id JOIN cycling_routes r ON r.id = a.route_id
